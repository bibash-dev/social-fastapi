...
from social_api.security import get_subject_for_token_type
...
def test_get_subject_for_token_type_wrong_type():
    ...
    with pytest.raises(security.HTTPException) as exc_info:
        security.get_subject_for_token_type(token, "access")
    assert (
        "Token has incorrect type: 'confirmation', expected 'access'"
...
@pytest.mark.anyio
async def test_get_users(registered_user: dict):
