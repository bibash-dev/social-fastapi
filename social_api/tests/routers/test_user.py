...
async def test_confirm_user(async_client: AsyncClient, mocker):
    ...
    confirmation_url = str(spy.call_args_list[1]["confirmation_url"])
...
async def test_confirm_user_invalid_token(async_client: AsyncClient):
    response = await async_client.get("/confirm_user/invalid_token")
    assert response.status_code == 401


@pytest.mark.anyio
async def test_confirm_user_token_expired(async_client: AsyncClient, mocker):
