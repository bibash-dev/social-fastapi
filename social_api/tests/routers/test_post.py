...
def mock_generate_cute_creature_api(mocker):
    return mocker.patch(
        "social_api.tasks.generate_cute_creature_api",
...
async def test_create_post_expired_token(
    ...
    mocker.patch(
        "social_api.security.access_token_expire_minutes",
        return_value=-1,
    )
...
async def test_create_comment(
    ...
    response = await async_client.post(
        ...
        headers={"Authorization": f"Bearer {created_post['token']}"},
    )
    assert response.status_code == 201
    assert {
        "id": 1,
...
async def test_get_missing_post_with_comments(
    ...
    response = await async_client.get(f"/post/20")
